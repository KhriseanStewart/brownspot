#!/usr/bin/env node
/**
 * Download edouard-claude/snip binaries into CLI/resources/snip/<platform>/
 * Used at postinstall / release so end users get token filtering.
 */
...
async function latestRelease() {
  const response = await fetch(`https://api.github.com/repos/${REPO}/releases/latest`, {
    headers: {
      ...
      "User-Agent": "brownspot-snip-download",
    },
  });
...
async function download(url, dest) {
  const response = await fetch(url, {
    headers: { "User-Agent": "brownspot-snip-download" },
  });
...
if (!version) throw new Error("Could not resolve the latest snip version");
...
for (const key of missing) {
  ...
  const work = join(tmpdir(), `brownspot-snip-${key}-${Date.now()}`);
  ...
  if (!extracted) throw new Error(`snip binary missing from ${name}`);
